...
const ListItem: React.FC<ListItemProps> = ({ href, image, name }) => {
  ...
  return (
    <button onClick={handleClick} className='relative group flex items-center rounded-md overflow-hidden gap-x-4 bg-neutral-100/10 hover:bg-neutral-100/20 transition pr-4'>
      ...
      <div
        className='absolute transition rounded-full bg-green-500 opacity-0 flex justify-center items-center group-hover:opacity-100 hover:scale-110 right-5 p-4 drop-shadow-md'
      >
